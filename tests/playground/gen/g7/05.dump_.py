...
def save_draw_schema(draw_model: MetroDrawSchema, output_path: str = "metro-draw-schema.json"):
    ...
    print(f"✅ 绘图Schema已保存到：{output_path}")
...
if __name__ == "__main__":
    ...
    # 步骤3：生成并保存Schema JSON
    save_draw_schema(draw_model, "metro-draw-schema.json")

    # 可选：打印精简模型示例（验证数据）
    print("\n📌 精简模型示例（第一条线路第一个站点）：")
    print(json.dumps(draw_model.lines[0].model_dump(), ensure_ascii=False, indent=2))