...
if __name__ == "__main__":
    p_playwright = """
你可以调用 执行 python 代码的工具，使用 Playwright 编写各种操作，并将结果页面截图保存下来png，然后调用 model="glm-4.6v" 视觉大语言模型观察，进一步推到怎么做。比如：碰到需要xxxxx网页要求用户登录的界面（通过"glm-4.6v"观察 Playwright 存储的图片），请编写代码，使用人机协作的方式，不仅要弹出浏览器让用户登录，还需要弹出弹出一个独立的桌面窗口，提示用户在浏览器中完成登录操作，登录完成后用户点击“我已登录，继续”按钮，程序才继续执行后续操作（注意需要 print 打印用户的选择， 方便自己调用 python exec 后拿到终端输出，可以判断用户的行为）。确保该桌面窗口不会因网页刷新而丢失，并且能够跨平台运行（Windows / macOS / Linux）。完成登录确认后，继续执行后续的网页操作，例如搜索商品并截图保存。 如下是参考代码：
"""
    p_hitl = r"""
...
async def main():
    async with async_playwright() as p:
        ...
        await page.goto("https://www.xxxxx.com", wait_until="domcontentloaded")
        ...
        # 2) 后续操作（示例：搜索并截图）
        await page.goto("https://search.xxxxx.com/Search?keyword=yyyyyy", wait_until="domcontentloaded")
        await page.wait_for_timeout(3000)
        await page.screenshot(path="xxxxx_yyyyyy.png", full_page=True)
...
根据以上提示，请你作为一个使用 GLM-V4.6 视觉大语言模型 的智能助手，帮助用户完成他们的查询任务。请确保你理解用户的需求，并根据提示中的信息提供准确和有用的回答。记得，得出答案的时候，需要告诉我是从哪个网页界面，把存下来图片的连接记得print或者告诉我。用户问题：
    ...
    p_user = "帮我查一下glm-asr-nano总共有多少下载量（hgf）和多少个star（github）"
    
    user_input = p_playwright + p_hitl + p_glmv46 + p_concat + p_user
    from src.runtime import subthread_python_executor
    subthread_python_executor.work_dir = './wsm/5glm/6hgf_github'
