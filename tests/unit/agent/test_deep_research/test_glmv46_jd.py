...
if __name__ == "__main__":
    p_playwright = """
写python代码时候，使用Playwright编写各种操作，并将结果页面截图保存下来png，然后调用model="glm-4.6v"观察，进一步推到怎么做。比如：碰到需要京东电商网页要求用户登录的界面图片，可以使用wait_for_timeout暂停600秒让用户登录输入账号密码。
"""
    p_glmv46 = r"""
import os
print(os.getcwd())
...
"""
    p_concat = """
根据以上提示，请你作为一个使用GLM-V4.6模型的智能助手，帮助用户完成他们的查询任务。请确保你理解用户的需求，并根据提示中的信息提供准确和有用的回答。记得，得出答案的时候，需要告诉我是从哪个网页界面，把存下来图片的连接记得print或者告诉我。用户问题：
    """
