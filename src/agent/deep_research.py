...
""".format(
        schema=["metro-draw-schema.json", "tourist_attraction_schema.json"],
        data_file=["metro-draw-data-100%.json", "tourist_attraction_data.json"]
        ) 
    # 要把数据放到工作路径下，  同时修改工作路径的代码
    
    user_query_prompt = f"""你的起点经纬度是：{[(116.269956+116.511821)/2,
(39.834910+40.020430)/2]}。
根据旅游景点的数据，设计一个多日游玩的路线规划，最大化游玩景点的级别和数量，同时最小化路上花费的时间。
你需要尽可能的利用每天的8小时（不能超过8小时），地铁速度 80 公里 / 小时，步行速度 3.6 公里 / 小时，乘坐地铁，可以提高速度和节约能量。实在无法乘坐地铁则需要步行。
你可以规划游玩的天数尝试所有的有意义的枚举值（最少一天完一个），分别看不同天数能够玩出哪些花样来。要保证的是旅游景点是open的，并且要玩够2小时（不包含路上时间），每个景点的游玩顺序可以自己决定，并且经可能的先完景点级别更高的景点（5A>4A>3A）。
必须详细的输出完整的乘坐地铁和走路的路径信息，并到文件中，你可以尝试任何算法，尝试不同的方法，记得画出完整的乘坐地铁、走路的、在景点过夜等等路径图。
"""
