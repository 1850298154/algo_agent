# [gen](https://www.doubao.com/chat/32170480107858434)
### 一、完整代码实现（生成数据结构+随机数据+Schema导出）
```python
import json
import random
...
### 二、数据结构说明（Markdown）
| 模型类名          | 字段名                | 字段类型       | 字段含义                                                                 | 取值说明                                                                 |
|-------------------|-----------------------|----------------|--------------------------------------------------------------------------|--------------------------------------------------------------------------|
| `Point`           | `x`                   | float          | 坐标点横坐标                                                             | 单位：米，随机生成范围0~1000，保留2位小数                                |
|                   | `y`                   | float          | 坐标点纵坐标                                                             | 单位：米，随机生成范围0~1000，保留2位小数                                |
| `Edge`            | `start`               | Point          | 道路边起点                                                               | 关联地图中的Point对象                                                    |
|                   | `end`                 | Point          | 道路边终点                                                               | 关联地图中的Point对象（与起点不同）|
|                   | `is_blocked`          | bool           | 道路是否阻断                                                             | True=阻断（无法通行），False=通行；20%概率生成阻断状态                   |
| `MapData`         | `points`              | List[Point]    | 地图所有坐标点列表                                                       | 默认生成10个随机坐标点                                                   |
|                   | `edges`               | List[Edge]     | 地图所有道路边列表                                                       | 默认生成15条随机道路边                                                   |
| `UAV`             | `uav_id`              | str            | 无人机唯一标识                                                           | 格式：UAV+3位数字（如UAV001）|
|                   | `load_capacity`       | int            | 无人机运载容量                                                           | 固定值1（单位：箱）|
| `UnmannedVehicle` | `uv_id`               | str            | 无人车唯一标识                                                           | 格式：UV+3位数字（如UV001）|
|                   | `position`            | Point          | 无人车当前位置                                                           | 随机生成坐标点                                                           |
|                   | `uav_capacity`        | int            | 无人车可携带无人机的最大数量                                             | 随机生成1~3之间的整数                                                    |
| `Material`        | `material_id`         | str            | 物资唯一标识                                                             | 格式：M+3位数字（如M001）|
|                   | `start_position`      | Point          | 物资起点位置（仓库坐标）| 固定为(500.0, 500.0)，可按需修改为随机                                   |
|                   | `target_position`     | Point          | 物资目标位置（受灾点坐标）| 随机生成坐标点                                                           |
| `AllData`         | `map_data`            | MapData        | 地图数据                                                                 | 包含所有坐标点和道路边                                                   |
|                   | `unmanned_vehicles`   | List[UnmannedVehicle] | 所有无人车数据 | 默认生成3辆无人车                                                       |
|                   | `uavs`                | List[UAV]      | 所有无人机数据                                                           | 默认生成8架无人机                                                       |
|                   | `materials`           | List[Material] | 所有物资数据                                                             | 默认生成10批物资                                                         |

### 三、代码执行说明
1. **运行环境**：Python 3.8+，需安装`pydantic`（执行`pip install pydantic`）；
2. **执行结果**：
   - 生成`data_schema.json`：包含所有数据结构的JSON Schema（字段类型、描述、约束等）；
   - 生成`random_data_1.json`/`random_data_2.json`/`random_data_3.json`：3份不同的随机测试数据，可直接用于算法测试；
3. **自定义调整**：
   - 修改`generate_random_map`的`num_points`/`num_edges`可调整地图点/边数量；
   - 修改`generate_unmanned_vehicles`的`num_uv`可调整无人车数量；
   - 修改`generate_uavs`的`num_uav`可调整无人机数量；
   - 修改`generate_materials`的`num_material`可调整物资数量；
   - 如需修改仓库起点，可调整`generate_materials`中的`warehouse_pos`。

### 四、数据使用示例
生成的JSON数据可直接读取用于规则化算法测试，例如：
```python
# 读取随机数据
