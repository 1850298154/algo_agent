...
项目主要使用 OR-Tools 的线性规划求解器 SCIP: [0-cite-0](#0-cite-0) 

**使用场景:**
- **MILP 任务分配** (`assign_uav_to_coalition_new`): 最小化 makespan,将智能体分配到联盟 [0-cite-1](#0-cite-1) 
- **c_milp_1 规划器**: VRP 风格的任务序列规划,使用 MTZ 约束消除子回路 [0-cite-2](#0-cite-2) 
- **c_milp_2 规划器**: 目标级别的智能体-目标分配 [0-cite-3](#0-cite-3) 
- **c_milp_1_agent_assign_seq_k**: 带序列约束的 VRP 规划 [0-cite-4](#0-cite-4) 
- **采样规划器** (`grow_sample_node`, `grow_sample_node_target`): 增量式任务分配 [0-cite-5](#0-cite-5) [0-cite-6](#0-cite-6) 
- **最小智能体需求计算** (`calc_min_agent_demand`): 计算任务所需最少智能体数量 [0-cite-7](#0-cite-7) 
- **运动规划** (`MILPlanner`): 多智能体路径规划 [0-cite-8](#0-cite-8) 

### 2. **SciPy**
使用 `scipy.optimize.linprog` 进行线性规划 [0-cite-9](#0-cite-9) 

**使用场景:**
- **LP 智能体分配** (`assign_uav_to_coalition`): 最小化旅行时间的线性规划 [0-cite-10](#0-cite-10) 

## 可视化工具

### 1. **Matplotlib**
主要的绘图库 [0-cite-11](#0-cite-11) 

**绘制的图表类型:**

#### (1) **堆叠面积图 (Stackplot)**
- `draw_stack_fig`: 显示智能体状态分布(导航/等待/执行)和任务数量随时间变化 [0-cite-12](#0-cite-12) 
- `draw_stack_fig_alpha`: 带任务完成率的智能体状态图 [0-cite-13](#0-cite-13) 

#### (2) **甘特图 (Gantt Chart)**
- `draw_task_gantt_fig`: 联盟任务执行时间线 [0-cite-14](#0-cite-14) 
- `draw_task_real_time_gantt_fig`: 任务实时执行阶段(发布/导航/同步/执行) [0-cite-15](#0-cite-15) 

#### (3) **直方图 (Histogram)**
- `draw_response_time_fig`: 响应时间分布 [0-cite-16](#0-cite-16) 

#### (4) **茎叶图 (Stem Plot)**
- `draw_planning_time_fig`: 重规划时间成本 [0-cite-17](#0-cite-17) 

#### (5) **实时仿真可视化**
在 `SimulationInterfaceMatplotlib` 和 `SimulationInterfaceMatplotlib2` 中: [0-cite-18](#0-cite-18) 
- **散点图 (Scatter)**: 智能体位置、目标位置 [0-cite-19](#0-cite-19) 
- **多边形/圆形填充**: 任务区域 [0-cite-20](#0-cite-20) 
- **轨迹线 (LineCollection)**: 历史轨迹渐变显示 [0-cite-21](#0-cite-21) 
- **规划轨迹线**: 未来计划路径 [0-cite-22](#0-cite-22) 
- **任务依赖图**: 显示任务前序和同步关系 [0-cite-23](#0-cite-23) 

### 2. **Plotly**
用于桑基图 [0-cite-24](#0-cite-24) 

**绘制的图表:**
- **桑基图 (Sankey Diagram)**: 显示智能体-联盟-任务的流动关系

### 3. **PyQtGraph**
用于 Qt 界面的高性能绘图 [0-cite-25](#0-cite-25) 

## 其他关键库

- **NetworkX**: 任务依赖图管理 [0-cite-26](#0-cite-26) 
- **NumPy**: 数值计算和矩阵操作 [0-cite-27](#0-cite-27) 
- **Shapely**: 几何对象操作 [0-cite-28](#0-cite-28) 
- **PyQt5**: GUI 框架 [0-cite-29](#0-cite-29) 
